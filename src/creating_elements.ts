const add_btn = document.getElementById('add_btn')! as HTMLButtonElement; // button for adding new todo
const new_todos_input = document.querySelector('.new_todos_input')! as HTMLInputElement; // input for entering new todo
const ulElement = document.getElementById('ulElement')! as HTMLUListElement; // HTML ul element is chosen
const defaultCheckbox = document.getElementById('default_checkbox')! as HTMLInputElement;
const defaultParent = defaultCheckbox.parentElement! as HTMLDivElement
const siblingElement = defaultParent?.nextElementSibling! as HTMLElement;

const defaultDeleteSVG = document.getElementById('delete')! as HTMLElement;
const defaultDeleteSVG_ParentParentElement = defaultDeleteSVG.parentElement?.parentElement! as HTMLLIElement;

const defaultEditSVG = document.getElementById('edit')! as HTMLElement;

function addNewTodo() {
    const newLi = document.createElement('li'); // created new li element
    newLi.classList.add('liElement');
    const checkbox_edit = document.createElement('div')! as HTMLDivElement;;// created new div element for checkbox and edit svg
    checkbox_edit.classList.add('checkbox_edit');
    const time_delete = document.createElement('div')! as HTMLDivElement;; // created new div element for time and delete svg
    time_delete.classList.add('time_delete');

    const checkbox = document.createElement('input')! as HTMLInputElement; checkbox.type = 'checkbox';
    ...
    const timeShow = document.createElement('h5')! as HTMLElement;
    ...
    function getParentElement() {
        const checkBoxParentElement = checkbox.parentElement! as HTMLDivElement;
        const h3Elemtnt = checkBoxParentElement?.nextElementSibling! as HTMLElement;
    ...
    function deleteParentElement() {
        const DeletesvgParentParentElement = deleteSVG.parentElement?.parentElement! as HTMLLIElement;
        deleteSVG.onclick = () => {
    ...
    function editTodoElement(){
        const editTodo = editSvg.parentElement?.nextElementSibling! as HTMLElement;
        editSvg.onclick = () => {
